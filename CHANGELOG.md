...
- Added search intents across the product:
  - `Best Match`
  - `What's New`
  - `Discovery`
- Added explicit search-horizon control for on-demand search:
  - `7d`
  - `30d`
  - `180d`
  - `1y`
  - `3y`
  - `5y`
- Added local search-first Home UI and local inbox flow as the default user path.
...
- Added topic-level relevance modes for stricter or broader matching behavior in the legacy digest path.
- Added Korean live preview page (`docs/preview/index_ko.html`) with the same live-search flow as the English page.

### Changed
- Daily digest delivery was kept as an optional layer instead of the core concept.
- GitHub live preview was upgraded from a simple mock/demo path into a real browser-run pipeline:
  - context -> query generation
  - arXiv / PubMed retrieval
  - intent-aware filtering
  - Gemini reranking
  - new-tab HTML result page
- Local LLM reranking was changed from small batch scoring to listwise reranking over the shortlisted candidate set.
- Preview retrieval now expands lookback windows adaptively and retries with broader fallback queries when needed.
- Gemini model fallback order was expanded so quota/model errors try multiple models before failing.
- Korean README/manuals were rewritten to match the search-first + local-inbox product direction.

### Docs
- Updated onboarding, sample env, README, and beginner docs to match the search-first workflow.
- Restored cumulative changelog history instead of keeping only the latest release entry.
